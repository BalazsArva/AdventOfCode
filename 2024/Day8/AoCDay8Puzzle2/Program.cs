        ...
        foreach (var antennaPositions in frequencyPositionLookup.Values)
        {
            for (var i = 0; i < antennaPositions.Count - 1; ++i)
            {
                for (var j = i + 1; j < antennaPositions.Count; ++j)
                {
                    ...
                    var vDistance = jRow - iRow;
                    var hDistance = jCol - iCol;

                    var (posRow, posCol) = (iRow, iCol);
                    while (isValidPosition(posRow, posCol))
                    {
                        hasNode[posRow, posCol] = true;
                        (posRow, posCol) = (posRow + vDistance, posCol + hDistance);
                    }
                    (posRow, posCol) = (iRow, iCol);
                    while (isValidPosition(posRow, posCol))
                    {
                        hasNode[posRow, posCol] = true;
                        (posRow, posCol) = (posRow - vDistance, posCol - hDistance);
